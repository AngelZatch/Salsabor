...
if($delta_steps > 0){ // Add sessions to the group
	...
	$teacher_id = $group_details["parent_prof_principal"];
	...
	$last_session_date = $db->query("SELECT cours_start, cours_end FROM cours WHERE cours_parent_id = $group_id ORDER BY cours_id DESC LIMIT 1")->fetch(PDO::FETCH_ASSOC);

	for($i = 1; $i <= $delta_steps; $i++){
		// We get the new dates of the sessions
		$days_added = $i * 7;
		$start_date = strtotime($last_session_date["cours_start"].'+'.$days_added.'DAYS');
		$end_date = strtotime($last_session_date["cours_end"].'+'.$days_added.'DAYS');
...
if($delta_steps < 0){ // Remove sessions from the group
	$limit = -1 * $delta_steps;
	$sessions_to_delete = $db->query("SELECT cours_id FROM cours WHERE cours_parent_id = $group_id ORDER BY cours_id DESC LIMIT $limit")->fetchAll(PDO::FETCH_COLUMN);

	foreach($sessions_to_delete as $session_id){
		$db->query("DELETE FROM cours WHERE cours_id = $session_id");
	}
	$last_session = $db->query("SELECT cours_end, cours_id FROM cours WHERE cours_parent_id = $group_id ORDER BY cours_id DESC LIMIT 1")->fetch(PDO::FETCH_ASSOC);
	$end_date = new DateTime($last_session["cours_end"]);
	$new_recurrence_end = $end_date->format("Y-m-d");
	updateRecurrenceEndDate($db, $group_id, $new_recurrence_end);
	echo $last_session["cours_id"];
}
