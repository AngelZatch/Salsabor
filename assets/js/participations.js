$(document).ready(function() {
    ...
    */
    ...
    var session_id = $(e.relatedTarget).data('session'),
        modal = $(this);
    var previous_value = "";
    var compare;
        ...
        for (var i = 0; i <= records_list.length; i++) {
            if (i == records_list.length) {
                ...
                if (records_list[i].count > 0) {
                    contents += "<a href='user_tasks.php?id=" + records_list[i].user_id + "' target='_blank'><span class='glyphicon glyphicon-list-alt float-right' title='" + records_list[i].count + " tâche(s) restantes à faire'></span></a>";
                }
...
function renderParticipationWide(participation, is_waypoint) {
    var participation_status, waypoint_class = "",
        contents = "",
        photo = "assets/images/logotype-white.png";
